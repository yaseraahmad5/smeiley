...
function roomkr() {
  $('#ops').children().remove();
  var rekker = $('#mkr');
  rekker.find(".rsave").hide(), rekker.find('.rdelete').hide(), rekker.find('.modal-title').text("إنشاء غرفه جديدة"), rekker.modal({
    backdrop: "static"
  }), rekker.find('.rtopic').val(""), rekker.find(".rabout").val(""), rekker.find('.rpwd').val(""), rekker.find('.rwelcome').val(""), rekker.find(".rmax").val(""), rekker.find('.picroom').attr('src', 'site/' + location.host + 'room.png').attr('onclick', 'sendpicroom();'), rekker.find('.rlike').val(""), rekker.find('.roomcolor').val(""), rekker.find('.roomcolor').css("background", "#000"), rekker.find(".colormsgroom").val(""), rekker.find('.colormsgroom').css('background', "#000"), rekker.find('.roomcolorpic').val(""), rekker.find('.roomcolorpic').css('background', '#000'), rekker.find('.rdel').prop('checked', false).parent().show(), rekker.find('.broadcast').prop('checked', false).parent().show(), rekker.find(".rmake").show().off().click(function () {
    if (rekker.find(".rtopic").val().trim().length <= 0) return alert('لا يمكن ترك اسم الغرفة فارغاً ');
    if (rekker.find('.rmax').val().trim().length <= 0 || rekker.find('.rmax').val() > 1e3 || rekker.find('.rmax').val() < 2) return alert("يجب ان يكون عدداعظاء الروم لا يزيد عن 40 او اقل من 2");
    send("room+", {
      topic: rekker.find('.rtopic').val(),
      about: rekker.find(".rabout").val(),
      welcome: rekker.find('.rwelcome').val(),
      pass: rekker.find('.rpwd').val(),
      color: rekker.find('input.colorrooms').val(),
      colormsgroom: rekker.find("input.colormsgroom").val(),
      colorpicroom: rekker.find('input.roomcolorpic').val(),
      max: Number(rekker.find(".rmax").val()),
      pic: rekker.find('.picroom').attr('src'),
      like: Number(rekker.find(".rlike").val()),
      delete: rekker.find('.rdel').prop('checked'),
      camera: false,
      broadcast: rekker.find('.broadcast').prop('checked')
    }), rekker.modal('hide');
  });
}
...
function redit(rainne) {
  $('#ops').children().remove();
  rainne == null && (rainne = M_ROOM);
  var ushna = R_CASH[rainne];
  if (ushna == null) return;
  var sarvani = $('#mkr');
  sarvani.find('.modal-title').text('إداره الغرفه'), sarvani.find(".rsave").show().off().click(function () {
    if (sarvani.find(".rtopic").val().trim().length <= 0) return alert('لا يمكن ترك اسم الغرفة فارغاً ');
    if (sarvani.find('.rmax').val().trim().length <= 0 || sarvani.find('.rmax').val() > 1e3 || sarvani.find(".rmax").val() < 2) return alert("يجب ان يكون عدداعضاء الروم لا يزيد عن 40 او اقل من 2");
    send('room^', {
      id: rainne,
      topic: sarvani.find('.rtopic').val(),
      about: sarvani.find('.rabout').val(),
      welcome: sarvani.find('.rwelcome').val(),
      broadcast: sarvani.find('.broadcast').prop('checked'),
      camera: false,
      pass: sarvani.find('.rpwd').val(),
      color: sarvani.find('input.colorrooms').val(),
      colormsgroom: sarvani.find('input.colormsgroom').val(),
      colorpicroom: sarvani.find("input.roomcolorpic").val(),
      pic: sarvani.find('.picroom').attr('src'),
      max: Number(sarvani.find('.rmax').val()),
      like: Number(sarvani.find('.rlike').val())
    }), $('#rooms').find(".r" + ushna.id + ' .u-topic').css('color', sarvani.find('input.roomcolor').val()), sarvani.modal('hide');
  }), $("#rooms").find(".r" + ushna.id + " .u-pic").css({
    border: '2px solid ' + sarvani.find('input.roomcolorpic').val()
  }), sarvani.modal("hide"), $("#rooms").find(".r" + ushna.id + ' .u-msg').css('color', sarvani.find('input.colormsgroom').val()), sarvani.modal('hide'), sarvani.find('.rdelete').show().off().click(function () {
    send("room-", {
      id: rainne
    }), sarvani.modal('hide');
  }), sarvani.modal({
    backdrop: 'modal',
    title: "ffff"
  }), sarvani.find('.rpwd').val(""), sarvani.find('.rtopic').val(ushna.topic), sarvani.find('.rabout').val(ushna.about), sarvani.find('.rwelcome').val(ushna.welcome), sarvani.find(".broadcast").prop('checked', ushna.broadcast), sarvani.find('.rmax').val(ushna.max), sarvani.find(".picroom").attr("src", ushna.pic), sarvani.find('.color').val(ushna.color), sarvani.find(".color").css('color', 'transparent'), sarvani.find('button.roomcolor').css('background-color', ushna.color), sarvani.find('.colorpicroom').val(ushna.colorpicroom), sarvani.find('.colorpicroom').css('colorpicroom', "transparent"), sarvani.find('button.roomcolorpic').css('background-color', ushna.colorpicroom), sarvani.find('.colormsgroom').val(ushna.colormsgroom), sarvani.find('.colormsgroom').css('colormsgroom', 'transparent'), sarvani.find('button.colormsgroom').css("background-color", ushna.colormsgroom), sarvani.find('.rlike').val(ushna.rmli), sarvani.find('.rmake').hide(), sarvani.find(".rdel").parent().hide(), send('ops', {});
}
